<?php
	...
	final class Headers {
		...
		public function __construct() {
			...
			if(method_exists(get_called_class(),self::$extension)):
		...
		private function css() {
			...
			if(preg_match('#/style/#',self::$file)):
				return;
			elseif(!is_file(ROOT_PATH.self::$file)):
				header('HTTP/1.0 404 Not Found');
				echo '/* file not found */';
				exit;
			else:
				$file = stat(ROOT_PATH.self::$file);
				...
				header('Content-Length: '.filesize(ROOT_PATH.self::$file));
				header('Cache-Control: public, max-age=86400');
				header('Pragma: ');
		...
		private function js() {
			...
			header('Content-Encoding: gzip');
			if(!is_file(ROOT_PATH.self::$file)):
				header('HTTP/1.0 404 Not Found');
				echo '/* file not found */';
				exit;
			else:
