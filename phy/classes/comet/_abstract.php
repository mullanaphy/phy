...
#####	# Magic methods.
		...
		final public function __construct($parameters=array(),$database=NULL) {
			$this->_settings['MySQL'] = is_object($database)?:\PHY\Registry::get('MySQL/default');
			if(is_object($parameters)) $this->_settings['parameters'] = \PHY\Convert::object_to_array($parameters);
		...
		final public function __toString() {
			return json_encode($this->_stack);
		}

		final public function __set($key,$value=NULL) {
			if(get_parent_class($this) === __CLASS__) $this->_settings[$key] = $value;
		}
