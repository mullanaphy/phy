...
#####	# Magic methods.		
		public function __destruct() { if($this->_socket!==false) fclose($this->_socket); }
...
#####	# Get methods.
		...
		public function getLastWarning() { return $this->_warning; }
		
		# get last error flag(to tell network connection errors from searchd errors or broken responses)
		public function isConnectError() { return $this->_connerror; }
...
#####	# Reset methods.	
		...
		public function ResetOverrides() { $this->_overrides = array(); }
		...
		public function addQuery($query,$index='*',$comment='') {
			...
			foreach($this->_overrides as $key => $entry)
			{
				...
				foreach($entry["values"] as $id=>$val)
				{
					...
					switch($entry["type"])
					{
						case self::ATTR_FLOAT:	$req .= $this->_packFloat($val); break;
						case self::ATTR_BIGINT:	$req .= $this->packI64($val); break;
...
#####	# Private methods.
		...
		private function _mbstringPop() { if($this->_mbenc) mb_internal_encoding($this->_mbenc); }
