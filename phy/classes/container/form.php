<?php
	class Container_Form extends Container_Abstract {
		const RECAPTCHA_KEY = '';
		...
		public function __construct($attributes=false) {
			$this->hidden['xsrf'] = (($_COOKIE['xsrf'])?$_COOKIE['xsrf']:NULL);
			$this->hidden['_form'] = 1;
		...
		public function captcha() {
					...
					$tag->table(
						$tag->tr(
							array(
											...
											$tag->tr(
												array(
													$tag->th(
														$tag->label(
															$tag->label('Solution:'),
															array('for' => 'recaptcha_response_field')
														),
								...
								$tag->td('CAPTCHAs help reduce spam by making sure the user is human (and not another computer). To solve a CAPTCHA, enter the two words from the image, separated by a space. {SITE_NAME} uses reCAPTCHA to stop spam and to help digitize old books and newspapers.',array('style' => 'padding:0 0 0 5px;'))
							)
						),
			...
			$this->append(
				$tag->script(
					'Recaptcha.create(\''.self::RECAPTCHA_KEY.'\',\''.$id.'\',{theme:\'custom\',custom_theme_widget:\'recaptcha_widget\'});',
			...
			if(isset($settings['url'],$settings['total'])&&$settings['total']>1):
				...
				$pages->prepend(
					$this->tag->li(
						...
						:$this->tag->span('&laquo;'),
						array('class'=>'button_dark')
					)
				);
				$pages->append(
					$this->tag->li(
						($settings['page_id']<$settings['total'])
						?$this->tag->url(
							'&raquo;',
							str_replace('[%i]',($settings['page_id']+1),$settings['url']),
							isset($settings['attributes'])?$settings['attributes']:NULL
		...
		protected function _footer() {
			...
			if($this->settings['submit']!==false):
				...
				$submit->append(
					$this->tag->input($this->settings['submit']),
					array('class' => 'submit')
				);
			endif;
